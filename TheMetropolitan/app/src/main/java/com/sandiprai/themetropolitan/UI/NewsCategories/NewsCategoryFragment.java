...
public class NewsCategoryFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        //Create the recycler
        RecyclerView newsArticlesRecycler = (RecyclerView) inflater.inflate(R.layout.fragment_saved,
                container, false);
        ...
        for(int i = 0; i < articleNames.length; i++){
            articleNames[i] = DummyArticle.articlesNews[i].getArticleTitle();
        }

        int[] articleImages = new int[DummyArticle.articles.length];
        for(int i = 0; i < articleImages.length; i++){
            articleImages[i] = DummyArticle.articlesNews[i].getImageId();
        }
        ...
        newsArticlesRecycler.setAdapter(dummyArticleAdapter);

        //Set the layout manager
        LinearLayoutManager layoutManager = new LinearLayoutManager(getActivity());
        newsArticlesRecycler.setLayoutManager(layoutManager);

        return newsArticlesRecycler;
    }
}
