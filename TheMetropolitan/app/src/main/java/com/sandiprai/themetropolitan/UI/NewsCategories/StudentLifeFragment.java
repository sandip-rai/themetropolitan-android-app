...
public class StudentLifeFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        //Create the recycler
        RecyclerView studentLifeArticlesRecycler = (RecyclerView) inflater.inflate(R.layout.fragment_saved,
                container, false);
        ...
        for(int i = 0; i < articleNames.length; i++){
            articleNames[i] = DummyArticle.articlesStudentLife[i].getArticleTitle();
        }

        int[] articleImages = new int[DummyArticle.articles.length];
        for(int i = 0; i < articleImages.length; i++){
            articleImages[i] = DummyArticle.articlesStudentLife[i].getImageId();
        }
        ...
        studentLifeArticlesRecycler.setAdapter(dummyArticleAdapter);

        //Set the layout manager
        LinearLayoutManager layoutManager = new LinearLayoutManager(getActivity());
        studentLifeArticlesRecycler.setLayoutManager(layoutManager);

        return studentLifeArticlesRecycler;
    }
}
