...
public class OpinionFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        //Create the recycler
        RecyclerView opinionArticlesRecycler = (RecyclerView) inflater.inflate(R.layout.fragment_saved,
                container, false);
        ...
        for(int i = 0; i < articleNames.length; i++){
            articleNames[i] = DummyArticle.articlesOpinion[i].getArticleTitle();
        }

        int[] articleImages = new int[DummyArticle.articles.length];
        for(int i = 0; i < articleImages.length; i++){
            articleImages[i] = DummyArticle.articlesOpinion[i].getImageId();
        }
        ...
        return opinionArticlesRecycler;
    }
}
